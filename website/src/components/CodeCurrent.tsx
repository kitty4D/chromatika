...
/** plausible snippets, chromatika / web3 flavored */
const SNIPPETS = [
  ...
  "Argon2id(salt, password, { t: 3, m: 65536, p: 4 })",
  ...
  useEffect(() => {
    ...
    function tick(now: number) {
      ...
      for (const d of drifters) {
        ...
        d.el.style.transform = `translate3d(${x}px, ${d.y + yBob}px, ${zPx}px) rotateY(${yawDeg}deg)`;
        d.el.style.opacity = String(opacity);
        if (parsedPalette.length > 1) {
          const pos =
            ((((d.age / colorCycleSec + d.colorPhase) % 1) + 1) % 1) * parsedPalette.length;
