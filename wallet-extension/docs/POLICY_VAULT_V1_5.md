...
>
> **2026-05-11 per-(vault, dwallet) update**: storage keys re-scoped from `<vaultId>` to `<vaultId>_<dwalletId>` (link, audit, presigns) so one chromatika vault can wrap multiple dWallets independently. ED25519 dWallets are now wrappable too (curve-aware dispatch picks the right per-(vault, dwallet) link at sign time via `session.dwalletMeta[curve].dwalletId`).

## What v1.5 EVM dispatch ships (2026-05-01)

- **`policy-vault-presigns.ts`**: per-(vault, dwallet) local cache of presign cap object ids (`chromatika_policy_presigns_v1_<vaultId>_<dwalletId>`; re-scoped from per-vault-only on 2026-05-11 alongside the multi-wrap refactor). Move uses `pop_back` (LIFO); chromatika appends on replenish, pops from the end on sign. Stays in sync via `resyncPolicyPresignsFromChain`.
