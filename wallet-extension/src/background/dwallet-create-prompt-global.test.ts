/**
 * Round-trip tests for the global "don't show on any vault" flag that suppresses
 * the empty-state `CreateDwalletPrompt` for every vault.
 *
...
describe('dwallet-create-prompt-global: global dismiss flag', () => {
  it('defaults to false when nothing is set', async () => {
    const m = await import('./dwallet-create-prompt-global');
    expect(await m.isDWalletCreatePromptGloballyDismissed()).toBe(false);
  });

  it('round-trips true through storage', async () => {
    const m = await import('./dwallet-create-prompt-global');
    await m.setDWalletCreatePromptGloballyDismissed(true);
    expect(await m.isDWalletCreatePromptGloballyDismissed()).toBe(true);
  });
  ...
  it('round-trips false to clear the flag', async () => {
    ...
    await m.setDWalletCreatePromptGloballyDismissed(true);
    expect(await m.isDWalletCreatePromptGloballyDismissed()).toBe(true);
    await m.setDWalletCreatePromptGloballyDismissed(false);
    expect(await m.isDWalletCreatePromptGloballyDismissed()).toBe(false);
  });
  ...
  it('rejects when chrome.runtime.lastError fires on set', async () => {
    ...
    await expect(m.setDWalletCreatePromptGloballyDismissed(true)).rejects.toThrow(/quota exceeded/);
  });
});
